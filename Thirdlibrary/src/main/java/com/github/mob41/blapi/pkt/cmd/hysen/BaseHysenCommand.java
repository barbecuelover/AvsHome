...
public abstract class BaseHysenCommand implements CmdPayload {
    ...
    public byte[] execute(BaseHysenDevice device) throws Exception {
        ...
        LogDebug(this.getClass().getSimpleName() + " received encrypted bytes: "
                );
        ...
        if (err == 0) {
            ...
            LogDebug(this.getClass().getSimpleName() + " received bytes (decrypted): ");
            return Arrays.copyOfRange(pl, 2, pl.length);
        } else {
            LogWarn(this.getClass().getSimpleName() + " received an error: " + Integer.toHexString(err) + " / " + err);
        }
        return null;
    }
