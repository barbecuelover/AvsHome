...
class LoginWithAmazonCBL extends Observable {
    ...
    LoginWithAmazonCBL(Context context,
        ...
        mClientId = mPreferences.getString(Constant.SpKeys.preference_client_id, "");
        mClientSecret = mPreferences.getString(Constant.SpKeys.preference_client_secret, "");
    ...
    void login() {
        Log.i(sTag, "Attempting to authenticate");
            ...
            try {
                if (!Pattern.matches(sDefaultRegExpr, mClientId)) {
                    ...
                    try {
                        ...
                        if (os != null) {
                            try {
                                ...
                                Log.w(sTag, "Cannot close resource. Error: "
                                        + e.getMessage());
                            }
                        }
                    }
                    ...
                    if (responseJSON != null) {
                        ...
                        Log.e(sTag, "Error requesting device authorization");
                    }

                } else {

                    Log.w(sTag, "Cannot authenticate. Invalid client ID");
                }
                    ...
                    if (i > 0) {
                        ...
                        try {
                            ...
                            if (responseCode == sResponseOk) {
                                ...
                                editor.putString(Constant.SpKeys.preference_refresh_token, refreshToken);
                                editor.putString(Constant.SpKeys.preference_login_method, LoginWithAmazon.CBL_LOGIN_METHOD_KEY);
                                editor.apply();
...
//                            mLogger.postDisplayCard(renderJSON, LogRecyclerViewAdapter.CBL_CODE_EXPIRED);
                        ...
                        Log.w(sTag, expiredMessage);
                    }
                }
            }, 0, sPollInterval * 1000);
        } catch (Exception e) {
            Log.e(sTag, "Error requesting device token. Error: " + e.getMessage());
        }
    }
    ...
    private void requestUserProfile(final String accessToken) {
        ...
        try {
            ...
            if (responseCode == HttpURLConnection.HTTP_OK) {
                ...
                if (responseJSON == null) {
                    Log.e(sTag, "Error requesting Token Info. Error: Null JSON Response");
                } else {
                    if (!mClientId.equals(responseJSON.getString("aud"))) {
                        // the access token does not belong to us
                        Log.e(sTag, "Error requesting Token Info. Error: Invalid access token");
                    } else {
                        try {
                            ...
                            if (responseCode == HttpURLConnection.HTTP_OK) {
                                try {
                                    ...
                                    if (responseJSON == null) {
                                        Log.e(sTag, "Error requesting User Profile. Error: null JSON Response");
                                    } else {
                                        Log.i(sTag, String.format("USER PROFILE: Name: %s, Email: %s, User ID: %s",
                                    ...
                                    Log.e(sTag, "Error requesting User Profile. Error: " + e.getMessage());
                                }
                            } else
                                Log.i(sTag, "User Profile request failed with code: " + responseCode);
                            if (urlConnection != null) {
                                urlConnection.disconnect();
                            }
                        } catch (Exception e) {
                            Log.e(sTag, "Error requesting Token Info. Error: " + e.getMessage());
                        }
                    }
                }
            } else {

                Log.i(sTag, "Token Info request failed with code: " + responseCode);
            }
        } catch (Exception e) {
            Log.e(sTag, "Error while requesting User Profile. Error: " + e.getMessage());
        }
    }
            ...
            if (!mRefreshToken.equals("")
                ...
                if (responseJSON != null) {
                    try {
                        ...
                        Log.e(sTag, "Error refreshing auth token. Error: "
                                + e.getMessage());
                    }
                    ...
                    Log.e(sTag, "Error refreshing auth token");
                }
    ...
    void logout() {
        Log.i(sTag, "Attempting to un-authenticate");
        ...
        SharedPreferences.Editor editor = mPreferences.edit();
        editor.putString(Constant.SpKeys.preference_refresh_token, "");
    ...
    void onInitialize() {
        String refreshToken = mPreferences.getString(Constant.SpKeys.preference_refresh_token, "");
